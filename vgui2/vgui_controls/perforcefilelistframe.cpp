...
// Sort by asset name
//-----------------------------------------------------------------------------
static int __cdecl OperationSortFunc( vgui2::ListPanel *pPanel, const vgui2::ListPanelItem &item1, const vgui2::ListPanelItem &item2 )
{
...
static int __cdecl FileBrowserSortFunc( vgui2::ListPanel *pPanel, const vgui2::ListPanelItem &item1, const vgui2::ListPanelItem &item2 )
{
...
// Constructor
//-----------------------------------------------------------------------------
COperationFileListFrame::COperationFileListFrame( vgui2::Panel *pParent, const char *pTitle, const char *pColumnHeader, bool bShowDescription, bool bShowOkOnly, int nDialogID ) :
	BaseClass( pParent, "PerforceFileList" )
{
	m_pText = NULL;
	vgui2::Panel *pBrowserParent = this;

	m_pDescription = NULL;
	m_pSplitter = NULL;
	if ( bShowDescription )
	{
		m_pSplitter = new vgui2::Splitter( this, "Splitter", vgui2::SPLITTER_MODE_HORIZONTAL, 1 );

		pBrowserParent = m_pSplitter->GetChild( 0 );
		vgui2::Panel *pDescParent = m_pSplitter->GetChild( 1 );

		m_pDescription = new vgui2::TextEntry( pDescParent, "Description" );
	...
	m_pFileBrowser = new vgui2::ListPanel( pBrowserParent, "Browser" );
 	m_pFileBrowser->AddColumnHeader( 0, "operation", "Operation", 52, 0 );
	m_pFileBrowser->AddColumnHeader( 1, "filename", pColumnHeader, 128, vgui2::ListPanel::COLUMN_RESIZEWITHWINDOW );
	...
	m_pYesButton = new vgui2::Button( this, "YesButton", "Yes", this, "Yes" );
	m_pNoButton = new vgui2::Button( this, "NoButton", "No", this, "No" );
...
//
//-----------------------------------------------------------------------------
CPerforceFileListFrame::CPerforceFileListFrame( vgui2::Panel *pParent, const char *pTitle, const char *pColumnHeader, PerforceAction_t action ) :
...
bool CPerforceFileListFrame::PerformOperation( )
{
	...
	case PERFORCE_ACTION_FILE_SUBMIT:
		{ 
			...
			{
				vgui2::MessageBox *pError = new vgui2::MessageBox( "Submission Error!", "Description required for submission.", GetParent() );
	...
	if ( !bSuccess )
	{
		vgui2::MessageBox *pError = new vgui2::MessageBox( "Perforce Error!", pErrorString, GetParent() );
		pError->SetSmallCaption( true );
		pError->DoModal();
	}
#if 0
	if ( *pErrorString )
	{
		...
		vgui2::MessageBox *pError = new vgui2::MessageBox( "Dubious Perforce Message", pMsg, GetParent() );
		pError->SetSmallCaption( true );
		pError->DoModal();
	}
#endif
...
//-----------------------------------------------------------------------------
void ShowPerforceQuery( vgui2::Panel *pParent, const char *pFileName, vgui2::Panel *pActionSignalTarget, KeyValues *pKeyValues, PerforceAction_t actionFilter )
{
	if ( !p4 )
	{
		...
		vgui2::ivgui()->PostMessage( pActionSignalTarget->GetVPanel(), pSpoofKeys, 0 );
