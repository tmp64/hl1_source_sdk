...
ScalableImagePanel::~ScalableImagePanel()
{
	...
	if ( vgui2::surface() && m_iTextureID != -1 )
	{
		vgui2::surface()->DestroyTextureID( m_iTextureID );
		m_iTextureID = -1;
	}
}
...
void ScalableImagePanel::PaintBackground()
{
	...
	for ( row=0;row<3;row++ )
	{
		...
		else
		{
			...
			uvh = max( 1.0f - 2 * m_flCornerHeightPercent, 0.0f );
		...
		for ( col=0;col<3;col++ )
		{
			...
			else
			{
				...
				uvw = max( 1.0f - 2 * m_flCornerWidthPercent, 0.0f );
			...
			vgui2::surface()->DrawTexturedPolygon( 4, verts );	
	...
	vgui2::surface()->DrawSetTexture(0);
}
