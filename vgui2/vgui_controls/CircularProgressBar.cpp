...
CircularProgressBar::~CircularProgressBar()
{
	for ( int i = 0; i < NUM_PROGRESS_TEXTURES; i++ )
	{
		if ( vgui2::surface() && m_nTextureId[i] )
		{
			vgui2::surface()->DestroyTextureID( m_nTextureId[i] );
			m_nTextureId[i] = -1;
		}
...
void CircularProgressBar::PaintBackground()
{
	...
	vgui2::surface()->DrawSetTexture( iTextureID );
...
void CircularProgressBar::DrawCircleSegment( Color c, float flEndProgress, bool bClockwise )
{
	...
	vgui2::surface()->DrawSetTexture( m_nTextureId[PROGRESS_TEXTURE_FG] );
	vgui2::surface()->DrawSetColor( c );
	...
	for ( int i=0;i<8;i++ )
	{
		if ( flEndProgressRadians > Segments[cur_wedge].minProgressRadians)
		{
			...
			vgui2::surface()->DrawTexturedPolygon( 3, v );
		}
